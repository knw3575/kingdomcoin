...
public class SPVBlockStore implements BlockStore {
    ...
    public SPVBlockStore(NetworkParameters params, File file) throws BlockStoreException {
            this(params, file, DEFAULT_NUM_HEADERS, HEADER_MAGIC);
    }
    /**
     * Creates and initializes an SPV block store. Will create the given file if it's missing. This operation
     * will block on disk.
     */
    public SPVBlockStore(NetworkParameters params, File file, int numHeaders, String headerMagic4Char) throws BlockStoreException {
        checkNotNull(file);
        checkState(headerMagic4Char.length() == 4, "header magic string must be 4 ascii characters.");
        ...
        try {
            ...
            if (exists) {
                ...
                buffer.get(header);
                if (!new String(header, "US-ASCII").equals(headerMagic4Char))
                    throw new BlockStoreException("Header bytes do not equal " + headerMagic4Char);
            } else {
                initNewStore(params, headerMagic4Char);
            }
        } catch (Exception e) {
            try {
    ...
    private void initNewStore(NetworkParameters params, String headerMagic) throws Exception {
        byte[] header;
        header = headerMagic.getBytes("US-ASCII");
        buffer.put(header);
