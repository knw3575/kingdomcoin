...
#if defined(NDEBUG)
# error "Kingdomcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x536c0e7ea569c228e892e86f2a02b2b9021f050682edd7683a00a982fc5791a5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kingdomcoin: starting difficulty is 1 / 2^12
...
{
    // Kingdomcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kingdomcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kingdomcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1481402709, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "2016";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1125 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e421bab90567ccbe9ab34bdb27d9fc87f1efcad5ba76754c75c25d434fa57de5aaa4e5095371d9bd120fec48a16e41d88e3039b750e30a02fdad50e7e5dd2c13") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1481402709;
        ...
        //assert(block.hashMerkleRoot == uint256("0xdce54a7da1c447a87d31d37d2cfa702ccf4371b50472070d86583e83dc139b8a"));
...
unsigned char pchMessageStart[4] = { 0x78, 0x80, 0xd2, 0xab }; // Kingdomcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KingdomcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KingdomcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KingdomcoinMiner(CWallet *pwallet)
{
    printf("KingdomcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running KingdomcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KingdomcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KingdomcoinMiner, pwallet));
}
